    ...
    return (
        <div className="space-y-1.5 w-full">
            ...
            <select
                id={id}
                {...props}
                className={`w-full h-11 bg-white/5 border border-white/10 rounded-xl px-4 text-white focus:outline-none focus:border-brand-500 transition-colors appearance-none ${props.className || ''}`}
            >
                <option value="" disabled className="bg-white text-black">Select {label}</option>
                {options.map((opt) => (
                    <option key={opt.value} value={opt.value} className="bg-white text-black py-1">{opt.label}</option>
                ))}
            </select>
        </div>
    );
